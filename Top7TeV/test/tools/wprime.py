...
# Setup TChain
treename = '/PATNtupleMaker/top'
if Lepton=="electron":
    treename = '/PATElectronNtupleMaker/top'
top = TChain(treename)
...
if Lepton=="muon":
    ...
    (dataType, dataTypeSuffix) = topSamples.eleSamples(top, dataType, dataTypeSuffix)
...
for jentry in xrange( entries ):
    ...
    if Lepton=="muon":
        ...
        if selector.ElectronZveto(p4ele):
            continue
        cutmap['ZVeto'] += 1
    ...
    if METzCalculator.IsComplex():
        hist.MET['LepWmassComplex'].Fill(p4LepW.M())
    else:
        hist.MET['LepWmass'].Fill(p4OtherLepW.M())
    ...
    if njets == 2:
        ...
        if WMt>30.:
            ...
            Wprime = p4LepW + p4Dijet
            hist.M3['Wprime'].Fill( Wprime.M() )

            if isTagb['SSVHEM'][0] or isTagb['SSVHEM'][1]:
                hist.M3['Wprime_1btag'].Fill( Wprime.M() )
            else:
                hist.M3['Wprime_0btag'].Fill( Wprime.M() )
            if isTagb['SSVHEM'][0] and isTagb['SSVHEM'][1]:
                hist.M3['Wprime_2btag'].Fill( Wprime.M() )
    ...
    if njets > 3:
        ...
        if not DissableMttbar:
            ...
            #top mass constraint
            myCombi.UsebTagging(False)
            myCombi.UseMtopConstraint(False)
            # choose sigmas
            #myCombi.SetSigmas(0);
            myCombi.SetLeptonicW(p4LepW)
            if p4OtherNu.E() != 0: myCombi.SetOtherLeptonicW(p4OtherLepW)
        ...
        if Lepton=="muon":
            ...
            hist.electrons['pt_4jet'].Fill( p4lepton.Pt() )
...
if Lepton=="electron":
    cutmapkeys =[ "CleanFilters","HLT","GoodPV","OneIsoMu","LooseMuVeto","ZVeto","ConversionVeto","MET","1Jet","2Jet","3Jet","4Jet"]
    if Flavor != 0:
        cutmapkeys =[ "CleanFilters","HLT","GoodPV","Flavor","OneIsoMu","LooseMuVeto","ZVeto","ConversionVeto","MET","1Jet","2Jet","3Jet","4Jet"]
