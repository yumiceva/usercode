...
#ifndef top_readskimh
#define top_readskimh
...
#ifdef top_readskim_cxx
top::top(TTree *tree)
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/uscms_data/d3/ttmuj/Documents/NtupleMaker/MC/V00-01-06/WJets_D6T_skim_bb.root");
      if (!f) {
         f = new TFile("/uscms_data/d3/ttmuj/Documents/NtupleMaker/MC/V00-01-06/WJets_D6T_skim_bb.root");
         f->cd("/PATNtupleMaker");
      }
      tree = (TTree*)gDirectory->Get("top");

      TTree *tree2 = (TTree*)gDirectory->Get("weight");
...
void top::Init(TTree *tree)
{
   ...
   fChain->SetBranchAddress("weight", &ntuple_weight);
   Notify();
}
